 ...
 */
export const selectSubscriptionPlan = asyncHandler(async (req, res, next) => {
  ...
  // Use placeholder dates for pending_payment - real dates set on activation
  const placeholderDate = new Date();

  // Create subscription record with pending_payment status
  // NOTE: startDate and endDate are placeholders only.
  // They will be recalculated when payment is confirmed (activateSubscription).
  const subscription = new AdminSubscription({
    ...
    endDate: placeholderDate, // Same as startDate — no countdown until paid
    autoRenew: false,
    usage: {
      hotels: 0,
      branches: 0,
...
export const activateSubscription = asyncHandler(async (req, res, next) => {
  ...
  if (subscription.status !== "pending_payment") {
    return next(new APIError(400, "This subscription is not pending payment"));
  }
...
export const getMySubscription = asyncHandler(async (req, res) => {
  ...
  // If no active, check for pending_payment
  let pendingSubscription = null;
  if (!subscription) {
    pendingSubscription = await AdminSubscription.findOne({
      admin: adminId,
      status: "pending_payment",
    })
      .populate("plan")
  ...
  // Return pending subscription info (but clearly mark it as not active)
  return res.status(200).json(
    new APIResponse(
      200,
      {
        ...
        pendingSubscription: {
          ...
          status: "pending_payment",
          message:
            "Payment is pending. Please complete payment to activate your subscription.",
        },
        hasSubscription: false,
      },
 ...
 */
export const getMyUsageStats = asyncHandler(async (req, res, next) => {
  const adminId = req.user._id;

  // Find only active (paid) subscription — pending_payment should NOT show usage
 ...
 * Cancels a pending_payment subscription when admin cancels/abandons payment
 * @route POST /api/v1/admin/subscription/cancel-pending
 */
export const cancelPendingPayment = asyncHandler(async (req, res, next) => {
  const adminId = req.user._id;

  // Find pending_payment subscription
  const subscription = await AdminSubscription.findOne({
    admin: adminId,
    status: "pending_payment",
  });
  ...
  if (!subscription) {
    return next(new APIError(404, "No pending payment found to cancel"));
  }
  ...
  subscription.paymentHistory.push({
    ...
    transactionId: `CANCEL-PENDING-${Date.now()}`,
    paymentDate: new Date(),
    status: "cancelled",
    notes: "Payment cancelled by admin before completion",
  });
  ...
  res.status(200).json(
    new APIResponse(
      ...
      "Pending payment cancelled successfully. You can now select a new plan."
    )
  );
});
 ...
 */
export const syncUsage = asyncHandler(async (req, res) => {
  const adminId = req.user._id;
  ...
  res
    .status(200)
    .json(new APIResponse(200, results, "Usage counters synced with database"));
});
