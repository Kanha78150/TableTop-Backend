...
export const bootstrapSuperAdmin = async (req, res, next) => {
  try {
    ...
    if (existingSuperAdmin) {
      console.log("⚠️  Bootstrap attempt blocked - Super Admin already exists");
      return next(
    ...
    const { error } = validateAdmin(req.body);
    if (error) {
      console.log("❌ Bootstrap validation failed:", error.details[0].message);
      return next(new APIError(400, error.details[0].message));
    }
    ...
    const existingAdmin = await Admin.findOne({ email });
    if (existingAdmin) {
      console.log("❌ Bootstrap failed - Email already exists:", email);
      return next(new APIError(409, "Admin with this email already exists"));
    }

    console.log("✅ Creating Super Admin:", { name, email, department });
    ...
    console.log("✅ Super Admin saved to database successfully");
