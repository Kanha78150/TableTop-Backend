...
// Mixed authentication: Allows both regular users AND admin/manager/staff
// Useful for endpoints that can be accessed by order owners OR hotel staff
export const authenticateAny = async (req, res, next) => {
  try {
    ...
    // Check if account is active (for admin/manager/staff)
    if (userType !== "user" && user.status !== "active") {
      throw new APIError(403, `${userType} account is inactive`);
    }
